...
public static class EntityGameManager
{
    ...
    /// <summary>
    /// 엔티티 처치 시 호출 (Entity.cs의 GiveScoreToLastAttacker에서 호출)
    /// </summary>
    public static void OnEntityKilled(int killerGameObjectId, int killedGameObjectId, string killedEntityName, int scoreGain = 100)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            // GameObject ID를 Entity ID로 변환
            if (gameObjectToEntityMap.TryGetValue(killerGameObjectId, out int killerEntityId))
            {
                bool updated = GameSessionManager.OnEntityKilled(killerEntityId, killedEntityName, scoreGain);

                if (updated)
                {
                    var sessionEntity = SessionEntityRepository.GetSessionEntity(GameSessionManager.GetCurrentSessionId(), killerEntityId);
            ...
            {
                Debug.LogWarning($"처치한 엔티티의 매핑을 찾을 수 없습니다. GameObject ID: {killerGameObjectId}");
            }

            // 처치당한 엔티티 매핑 제거
            if (gameObjectToEntityMap.TryGetValue(killedGameObjectId, out int killedEntityId))
            {
                gameObjectToEntityMap.Remove(killedGameObjectId);
                entityToGameObjectMap.Remove(killedEntityId);
            }
        }
        catch (System.Exception ex)
        {
    ...
    /// </summary>
    public static void OnEntityScoreAdd(int gameObjectInstanceId, int newScore, int newLevel = -1)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            {
                bool added = GameSessionManager.AddEntityScore(entityId, newScore, newLevel);

                if (!added)
                {
                    Debug.LogWarning($"엔티티 점수 추가가 실패했습니다. Entity ID: {entityId}");
                }
            }
            else
            {
                Debug.LogWarning($"엔티티 매핑을 찾을 수 없습니다. GameObject ID: {gameObjectInstanceId}");
            }
        }
        catch (System.Exception ex)
        {
    ...
    /// <summary>
    /// 엔티티 점수 업데이트 (Entity.cs의 AddScore에서 호출)
    /// </summary>
    public static void OnEntityScoreUpdate(int gameObjectInstanceId, int newScore, int newLevel = -1)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            if (gameObjectToEntityMap.TryGetValue(gameObjectInstanceId, out int entityId))
            {
                bool updated = GameSessionManager.UpdateEntityScore(entityId, newScore, newLevel);

                if (!updated)
                {
                    Debug.LogWarning($"엔티티 점수 업데이트에 실패했습니다. Entity ID: {entityId}");
                }
            }
            else
            {
    ...
    /// <summary>
    /// 플레이어 점수 업데이트 (Player.cs에서 호출)
    /// </summary>
    public static void OnPlayerScoreUpdate(int newScore, int newLevel = -1)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            bool updated = GameSessionManager.UpdatePlayerScore(newScore, newLevel);

            if (!updated)
            {
                Debug.LogWarning("플레이어 점수 업데이트에 실패했습니다.");
            }
        }
        catch (System.Exception ex)
        {
    ...
    /// 플레이어 점수 추가
    /// </summary>
    public static void OnPlayerScoreAdd(int newScore, int newLevel = -1)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            bool updated = GameSessionManager.AddPlayerScore(newScore, newLevel);

            if (!updated)
            {
                Debug.LogWarning("플레이어 점수 추가가 실패했습니다.");
            }
        }
        catch (System.Exception ex)
        {
            Debug.LogError($"플레이어 점수 추가 오류: {ex.Message}");
        }
    }
    ...
    /// <summary>
    /// 엔티티 레벨업 처리 (Entity.cs의 levelup에서 호출)
    /// </summary>
    public static void OnEntityLevelUp(int gameObjectInstanceId, int newLevel)
    {
        try
        {
            if (!GameSessionManager.HasActiveSession())
            ...
            if (gameObjectToEntityMap.TryGetValue(gameObjectInstanceId, out int entityId))
            {
                var sessionEntity = SessionEntityRepository.GetSessionEntity(GameSessionManager.GetCurrentSessionId(), entityId);
                if (sessionEntity != null)
                {
                    bool updated = GameSessionManager.UpdateEntityScore(entityId, sessionEntity.Score, newLevel);

                    if (updated)
                    {
                        Debug.Log($"{sessionEntity.EntityName} 레벨업! 새로운 레벨: {newLevel}");
                    }
                }
            }
        }
        ...
        {
            Debug.LogError($"엔티티 레벨업 처리 오류: {ex.Message}");
        }
    }

    /// <summary>
    /// 플레이어 레벨업 처리 (Player.cs의 levelup에서 호출)
    /// </summary>
    public static void OnPlayerLevelUp(int newLevel)
    {
        try
        {
            ...
            if (sessionEntity != null)
            {
                ...
                if (updated)
                {
                    Debug.Log($"플레이어 레벨업! 새로운 레벨: {newLevel}");
                }
            }
        }
        catch (System.Exception ex)
        {
            Debug.LogError($"플레이어 레벨업 처리 오류: {ex.Message}");
        }
    }
    ...
    public static List<SessionEntityModel> GetCurrentSessionEntities(bool aliveOnly = false)
    {
        if (!GameSessionManager.HasActiveSession())
        {
            return new List<SessionEntityModel>();
        }

        return GameSessionManager.GetCurrentSessionEntities(aliveOnly);
    }

    /// <summary>
    /// 매핑된 엔티티 수 반환 (디버그용)
    /// </summary>
    public static int GetMappedEntityCount()
    {
        return gameObjectToEntityMap.Count;
    }
    ...
    {
        Debug.Log($"현재 매핑된 엔티티 수: {gameObjectToEntityMap.Count}");
        foreach (var mapping in gameObjectToEntityMap)
        {
            Debug.Log($"GameObject ID: {mapping.Key} → Entity ID: {mapping.Value}");
        }
    }
}
